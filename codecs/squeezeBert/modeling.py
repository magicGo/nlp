    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            with tf.variable_scope("encoder"):
                ...
                tf.logging.info("ln_type: %s" % ln_type)
                if ln_type == 'postln' or ln_type is None:  # currently, base or large of albert used post-LN structure
                    tf.logging.info("old structure of transformer.use: transformer_model,which use post-LN")
                    ...
                    tf.logging.info("new structure of transformer.use: prelln_transformer_model,which use pre-LN")
                    self.all_encoder_layers = prelln_transformer_model(
                        ...
                        do_return_all_layers=True,
                        shared_type='all',
                        groups=config.groups)  # do_return_all_layers=True
...
def embedding_lookup_factorized(input_ids,  # Factorized embedding parameterization provide by albert
    ...
    tf.logging.info("embedding_lookup_factorized. factorized embedding parameterization is used.")
    if input_ids.shape.ndims == 2:
        input_ids = tf.expand_dims(input_ids, axis=[-1])  # shape of input_ids is:[ batch_size, seq_length, 1]
...
def attention_layer(from_tensor,
    ...
    from_tensor_3d = reshape_from_matrix(from_tensor, from_shape)
    to_tensor_3d = reshape_from_matrix(to_tensor, to_shape)

    # `query_layer` = [B*F, N*H]
    query_layer = tf.layers.conv1d(
        from_tensor_3d,
        num_attention_heads * size_per_head,
        ...
        with tf.variable_scope(name_variable_scope,
            ...
            with tf.variable_scope("attention"):
                ...
                with tf.variable_scope("output"):
                    attention_output = tf.layers.conv1d(
                        attention_output,
                        hidden_size,
                        kernel_size=1,
                        strides=1,
            ...
            attention_output_shape = get_shape_list(attention_output, expected_rank=3)
            group_in_size = int(attention_output_shape[2] / groups)
            group_out_size = int(intermediate_size / groups)
            with tf.variable_scope("intermediate"):
                intermediate_group_outputs = []
                for group_index in range(groups):
                    intermediate_group_output = tf.layers.conv1d(
                        attention_output[:, :, group_index * group_in_size: (group_index + 1) * group_in_size],
                        group_out_size,
                        kernel_size=1,
                        strides=1,
                    ...
                    intermediate_group_outputs.append(intermediate_group_output)
                intermediate_output = tf.concat(intermediate_group_outputs, axis=-1)

            # Down-project back to `hidden_size` then add the residual.
            intermediate_output_shape = get_shape_list(intermediate_output, expected_rank=3)
            group_in_size = int(intermediate_output_shape[2] / groups)
            group_out_size = int(hidden_size / groups)
            with tf.variable_scope("output"):
                output_group_outputs = []
                for group_index in range(groups):
                    layer_output = tf.layers.conv1d(
                        intermediate_output[:, :, group_index * group_in_size: (group_index + 1) * group_in_size],
                        group_out_size,
                        kernel_size=1,
                        strides=1,
                        kernel_initializer=create_initializer(initializer_range)
                    )
                    output_group_outputs.append(layer_output)
                layer_output = tf.concat(output_group_outputs, axis=-1)
...
def prelln_transformer_model(input_tensor,
    ...
    for layer_idx in range(num_hidden_layers):
        ...
        with tf.variable_scope(idx_scope['layer'], reuse=tf.AUTO_REUSE):
            ...
            # The activation is only applied to the "intermediate" hidden layer.
            attention_output_pre_shape = get_shape_list(attention_output_pre, expected_rank=3)
            group_in_size = int(attention_output_pre_shape[2] / groups)
            group_out_size = int(intermediate_size / groups)
