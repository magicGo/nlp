...
def freeze_session2(checkpoint_dir, bak_dir, session, model):
    ...
    with tf.gfile.GFile(os.path.join(bak_dir, 'inputs.yml'), 'w') as output:
    ...
    with tf.gfile.GFile(os.path.join(bak_dir, 'outputs.yml'), 'w') as output:
