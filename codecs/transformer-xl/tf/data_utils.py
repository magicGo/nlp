...
class Corpus(object):
    def __init__(self, path, dataset, *args, **kwargs):
        ...
        if self.dataset in ["ptb", "wt2", "wt103"]:
            ...
            self.valid = self.vocab.encode_file(
                valid_path, ordered=True, add_double_eos=True, verbose=True)
            self.test = self.vocab.encode_file(
                test_path, ordered=True, add_double_eos=True, verbose=True)
                ...
                if FLAGS.num_procs > 1:
                    ...
                    for shard, path in enumerate(self.train):
                        data_shard = self.vocab.encode_file(path, ordered=False,
                                                            add_double_eos=True, verbose=True)
...
if __name__ == "__main__":
    FLAGS = flags.FLAGS
    flags.DEFINE_string("data_dir", "sota/pretrained_xl/tf_enwik8/data",
                        help="Location of the data corpus")
    flags.DEFINE_enum("dataset", "enwik8",
    ...
    flags.DEFINE_integer("per_host_test_bsz", 16,
