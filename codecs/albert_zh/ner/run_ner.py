...
## Required parameters
flags.DEFINE_string(
    "data_dir", 'data/ner/heater/decreaseVolume',
...
flags.DEFINE_string(
    "output_dir", 'output/ner/heater/decreaseVolume',
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("loss"):
        ...
        small = tf.constant(-1000.0, dtype=tf.float32)
        batch_size = tf.shape(input_ids)[0]
        num_steps = tf.shape(input_ids)[-1]
        # pad logits for crf loss
        start_logits = tf.concat(
            [small * tf.ones(shape=[batch_size, 1, num_labels]), tf.zeros(shape=[batch_size, 1, 1])], axis=-1)
        pad_logits = tf.cast(small * tf.ones([batch_size, num_steps, 1]), tf.float32)
        logits = tf.concat([project_logits, pad_logits], axis=-1)
        logits = tf.concat([start_logits, logits], axis=1)
        targets = tf.concat([tf.cast(num_labels * tf.ones([batch_size, 1]), tf.int32), labels], axis=-1)

        trans = tf.get_variable(
            "transitions",
            shape=[num_labels + 1, num_labels + 1],
            initializer=tf.contrib.layers.xavier_initializer())
        log_likelihood, trans = tf.contrib.crf.crf_log_likelihood(
            inputs=logits,
            tag_indices=targets,
            transition_params=trans,
            sequence_lengths=mask2len + 1)

        loss = tf.reduce_mean(-log_likelihood)

        predicts, viterbi_score = tf.contrib.crf.crf_decode(logits, trans, mask2len + 1)

        predicts = predicts[:, 1: ]

    return loss, logits, predicts, trans, mask2len
...
def main(_):
    ...
    handlers = [logging.FileHandler(os.path.join(FLAGS.output_dir, nowTime + '.log'), encoding='utf-8')]
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer, tf.gfile.GFile(trie_file, "w") as fp:
            ...
            for (i, prediction) in enumerate(result):
                ...
                if i >= num_actual_predict_examples:
                    break
                output_line = text + t_label + p_label + '\n'
