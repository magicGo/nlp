...
class DataProcessor(object):
    ...
    def _read_tsv(cls, input_file, quotechar=None):
        ...
        with tf.gfile.Open(input_file, "r") as f:
            lines = []
            for line in f:
                label = line.split('[CLS]')[0].strip()
                query = line.split('[CLS]')[1].strip().split('[SEP]')[0].strip()
                tag = line.split('[CLS]')[1].strip().split('[SEP]')[1].strip()
                lines.append([query, label, tag])
            return lines
...
class HaierProcessor(DataProcessor):
    ...
    def __init__(self, data_dir, output_dir, do_train=None, do_eval=None, do_predict=None):
        ...
        if tf.gfile.Exists(os.path.join(self.output_dir, 'label.json')):
            ...
            with tf.gfile.GFile(os.path.join(self.output_dir, 'tag.json'), 'r') as f:
                self.inv_tag_map = json.load(f)
                self.tag_map = {label: int(index) for index, label in self.inv_tag_map.items()}
    ...
    def get_inv_tag_map(self):
        """See base class."""
        return self.inv_tag_map
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if set_type == 'train':
                ...
                for l in tag.split():
                    tags.add(l)
            examples.append(InputExample(guid=guid, text_a=text_a, label=label, tag=tag))
        shuffle(examples)
        ...
        if tf.gfile.Exists(os.path.join(self.output_dir, 'tag.json')):
            with tf.gfile.GFile(os.path.join(self.output_dir, 'tag.json'), 'r') as f:
                self.inv_tag_map = json.load(f)
                self.tag_map = {tag: int(index) for index, tag in self.inv_tag_map.items()}
        else:
            if set_type == 'train':
                tags = list(tags)
                tags.sort()
                tags.append('[CLS]')
                self.tag_map = {tag: index for index, tag in enumerate(tags)}
                self.inv_tag_map = {index: tag for tag, index in self.tag_map.items()}
                with tf.gfile.GFile(os.path.join(self.output_dir, 'tag.json'), 'w') as f:
                    f.write(json.dumps(self.inv_tag_map, ensure_ascii=False, indent=4))

        return examples


def convert_single_example(ex_index, example, label_map, tag_map, max_seq_length,
                           tokenizer):
    ...
    for i, (word, tag) in enumerate(zip(textlist, taglist)):
    ...
    tag_ids.append(tag_map["[CLS]"])
    for index, token in enumerate(tokens):
        ntokens.append(token)
        segment_ids.append(0)
        tag_ids.append(tag_map[tags[index]])
...
def file_based_convert_examples_to_features(examples, label_map, tag_map, max_seq_length, tokenizer, output_file):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example(ex_index, example, label_map, tag_map, max_seq_length, tokenizer)
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, num_labels, tags, num_tags, use_one_hot_embeddings):
    ...
    # instead.
    classifier_output_layer = model.get_pooled_output()
    classifier_hidden_size = classifier_output_layer.shape[-1].value
    ner_output_layer = model.get_sequence_output()

    with tf.variable_scope("classifier_loss"):
        output_weights = tf.get_variable(
            "output_weights", [num_labels, classifier_hidden_size],
        ...
        if ln_type == 'preln':  # add by brightmart, 10-06. if it is preln, we need to an additonal layer: layer normalization as suggested in paper "ON LAYER NORMALIZATION IN THE TRANSFORMER ARCHITECTURE"
            ...
            classifier_output_layer = layer_norm(classifier_output_layer)
        else:
            tf.logging.info("ln_type is postln or other,do nothing.")

        if is_training:
            # I.e., 0.1 dropout
            classifier_output_layer = tf.nn.dropout(classifier_output_layer, keep_prob=0.9)

        classifier_logits = tf.matmul(classifier_output_layer, output_weights, transpose_b=True)
        classifier_logits = tf.nn.bias_add(classifier_logits, output_bias)
        classifier_probabilities = tf.nn.softmax(classifier_logits, axis=-1)
        classifier_log_probs = tf.nn.log_softmax(classifier_logits, axis=-1)

        classifier_one_hot_labels = tf.one_hot(labels, depth=num_labels, dtype=tf.float32)

        classifier_per_example_loss = -tf.reduce_sum(classifier_one_hot_labels * classifier_log_probs, axis=-1)  # todo 08-29 try temp-loss
        ...
        classifier_loss = tf.reduce_mean(classifier_per_example_loss)
    ...
    with tf.variable_scope("ner_loss"):
        ...
        if ln_type == 'preln':  # add by brightmart, 10-06. if it is preln, we need to an additonal layer: layer normalization as suggested in paper "ON LAYER NORMALIZATION IN THE TRANSFORMER ARCHITECTURE"
            tf.logging.info("ln_type is preln. add LN layer.")
            ner_output_layer = layer_norm(ner_output_layer)
        else:
            tf.logging.info("ln_type is postln or other,do nothing.")

        if is_training:
            # I.e., 0.1 dropout
            ner_output_layer = tf.nn.dropout(ner_output_layer, keep_prob=0.9)

        ner_project_logits = tf.keras.layers.Dense(
            units=num_tags,
            activation=None,
        ...
        ner_project_logits = tf.reshape(ner_project_logits, [-1, FLAGS.max_seq_length, num_tags])
        ...
        ner_predicts, viterbi_score = tf.contrib.crf.crf_decode(ner_project_logits, trans, mask2len)

        ner_predicts = ner_predicts[:,: ]

    with tf.name_scope('total_loss'):
        loss = classifier_loss + ner_loss

    return loss, ner_project_logits, ner_predicts, trans, mask2len, classifier_per_example_loss, classifier_logits, classifier_probabilities
...
def model_fn_builder(bert_config, num_labels, num_tags, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        (total_loss, ner_logits, ner_predicts, trans, mask2len, classifier_per_example_loss, classifier_logits, classifier_probabilities) = create_model(
            bert_config, is_training, input_ids, input_mask, segment_ids, label_ids,
            num_labels, tag_ids, num_tags, use_one_hot_embeddings)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids, tag_ids, ner_predictions, num_labels, num_tags, input_mask, is_real_example):
                classifier_predictions = tf.argmax(classifier_logits, axis=-1, output_type=tf.int32)
                classifier_accuracy = tf.metrics.accuracy(labels=label_ids, predictions=classifier_predictions, weights=is_real_example)
                classifier_precision = tf_metrics.precision(label_ids, classifier_predictions, num_labels, average="macro")
                classifier_recall = tf_metrics.recall(label_ids, classifier_predictions, num_labels, average="macro")
                classifier_f1_score = tf_metrics.f1(label_ids, classifier_predictions, num_labels, average="macro")

                ner_accuracy = tf.metrics.accuracy(labels=tag_ids, predictions=ner_predictions, weights=is_real_example)
                ner_precision = tf_metrics.precision(tag_ids, ner_predictions, num_tags, average="macro")
                ner_recall = tf_metrics.recall(tag_ids, ner_predictions, num_tags, average="macro")
                ner_f1_score = tf_metrics.f1(tag_ids, ner_predictions, num_tags, average="macro")
                return {
                    "classifier_eval_accuracy": classifier_accuracy,
                    "classifier_eval_precision": classifier_precision,
                    "classifier_eval_recall": classifier_recall,
                    "classifier_eval_f1_score": classifier_f1_score,
                    "ner_eval_accuracy": ner_accuracy,
                    "ner_eval_precision": ner_precision,
                    "ner_eval_recall": ner_recall,
                    "ner_eval_f1_score": ner_f1_score
                }
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
    ...
    for feature in features:
        ...
        all_tag_ids.append(feature.tag_ids)
    ...
    def input_fn(params):
        ...
        d = tf.data.Dataset.from_tensor_slices({
            ...
            "tag_ids":
                tf.constant(all_tag_ids, shape=[num_examples], dtype=tf.int32),
        })
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        if not train_file_exists:  # if tf_record file not exist, convert from raw text file. # TODO
            file_based_convert_examples_to_features(train_examples, label_map, tag_map, FLAGS.max_seq_length, tokenizer, train_file)
        ...
        file_based_convert_examples_to_features(dev_examples, label_map, tag_map, FLAGS.max_seq_length, tokenizer, eval_file)
        ...
        early_stopping_hook = stop_if_no_increase_hook(
            estimator=estimator,
            metric_name='classifier_eval_accuracy_and_ner_eval_f1_score',
        ...
        logging_hook = tf.train.LoggingTensorHook(tensors={"train loss": "total_loss/add:0"}, every_n_iter=1)
    ...
    if FLAGS.do_eval:
        ...
        file_based_convert_examples_to_features(
            test_examples, label_map, tag_map, FLAGS.max_seq_length, tokenizer, eval_file)
