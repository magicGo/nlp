    ...
    def stop_if_no_metric_improvement_fn():
        ...
        for step, metrics in eval_results.items():
            ...
            if best_val is None or is_lhs_better(val, best_val):
                best_val = val
                best_val_step = step
            if step - best_val_step >= max_steps_without_improvement or best_val == 1.0:
                if best_val_step and best_val and current_step:
                    tf.logging.info('best step is: {}, eval {} is {}, current steps is {}'.format(
                        best_val_step,
                        metric_name,
                        best_val,
                        current_step
