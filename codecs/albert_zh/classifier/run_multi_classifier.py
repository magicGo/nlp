...
flags.DEFINE_string(
    "output_dir", 'output/intent/heater',
    ...
    def __init__(self, data_dir, output_dir, do_train=None, do_eval=None, do_predict=None):
        ...
        if data_dir:
            if do_train:
                ...
                # self.train_examples = self._create_examples(self.data[: int(len(self.data) * 0.7)], "train")
                # self.dev_examples = self._create_examples(self.data[int(len(self.data) * 0.7):], "dev")
        ...
        for (i, line) in enumerate(lines):
            ...
            if set_type == 'train':
                for l in label.split():
                    labels.add(l.strip())
            examples.append(
                InputExample(guid=guid, text_a=text_a, label=label.split()))
        shuffle(examples)
        if set_type == 'train':
            if tf.gfile.Exists(os.path.join(self.output_dir, 'label.json')):
                with tf.gfile.GFile(os.path.join(self.output_dir, 'label.json'), 'r') as f:
...
def convert_single_example(ex_index, example, label_map, max_seq_length,
    ...
    # label_id = [int(label_map[la]) for la in example.label]
    label_id = [0 for i in range(len(label_map))]
    for la in example.label:
        label_id[int(label_map[la])] = 1
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, probabilities, is_real_example):
                one = tf.ones_like(label_ids)
                zero = tf.zeros_like(label_ids)
                # # 小于0.5为0，大于0.5为1
                predictions = tf.where(probabilities < 0.5, x=zero, y=one)
...
def main(_):
    ...
    handlers = [logging.FileHandler(os.path.join(FLAGS.output_dir, nowTime + '.log'), encoding='utf-8'), logging.StreamHandler()]
    ...
    if FLAGS.do_predict:
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer, tf.gfile.GFile(trie_file, "w") as fp:
            ...
            for (i, prediction) in enumerate(result):
                ...
                t_label.sort()
                t_label = ' '.join(t_label)
                p_label = []
                p_score = []
                for index, prob in enumerate(probabilities):
                    if prob > 0.5:
                        p_label.append(processor.get_inv_label_map()[str(index)])
                        p_score.append(prob)
                if i >= num_actual_predict_examples:
                    break
                output_line = text + '\tt_label: ' + ' '.join(t_label) + '\tp_label: ' + ' '.join(p_label) + '\tp_score: ' + str(p_score) + '\n'
